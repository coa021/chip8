...
inline constexpr std::array<uint8_t, FONT_CHAR_COUNT * FONT_SPRITE_HEIGHT>
FONT_SET{
    {
...
public:
  ...
  [[nodiscard]] constexpr auto operator<=>(const StrongType &) const noexcept
  = default;
  [[nodiscard]] constexpr bool operator==(const StrongType &) const noexcept
  = default;
  ...
  [[nodiscard]] constexpr StrongType operator+(
      const StrongType &other) const noexcept {
